...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            <x-ui.button href="{{ route('admin.events.index') }}" variant="secondary">Back to Events</x-ui.button>
            @can('create', App\Models\EventCategory::class)
                <x-ui.button href="{{ route('admin.event-categories.create') }}">Add Category</x-ui.button>
            @endcan
        </div>
    </div>

    @if ($categories->isEmpty())
        <x-ui.empty-state heading="No categories yet" message="Create your first event category to get started." />
    @else
        <ul
            x-data="{
                saving: false,
                async saveOrder() {
                    ...
                    await fetch('{{ route('admin.event-categories.reorder') }}', {
                        method: 'POST',
                        headers: {
            ...
            @foreach ($categories as $category)
                ...
                >
                    <div class="flex items-center gap-3">
                        ...
                        <x-ui.badge variant="neutral">{{ $category->events_count }} {{ Str::plural('event', $category->events_count) }}</x-ui.badge>
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('admin.event-categories.edit', $category) }}" class="text-sm font-medium text-primary-700 hover:underline dark:text-night-text">
                            Edit
                        </a>

                        @if ($category->events_count === 0)
                            <form method="POST" action="{{ route('admin.event-categories.destroy', $category) }}" onsubmit="return confirm('Delete {{ $category->name }}?');">
                                @csrf
                                @method('DELETE')
