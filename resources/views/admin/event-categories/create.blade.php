...
@section('breadcrumbs')
    <x-ui.breadcrumbs :items="[
        ...
        ['label' => 'Categories', 'url' => route('admin.event-categories.index')],
        ['label' => 'Add'],
    ]" />
@endsection
...
@section('content')
    <x-ui.card class="max-w-xl">
        <form method="POST" action="{{ route('admin.event-categories.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="flex gap-3">
                ...
                <x-ui.button href="{{ route('admin.event-categories.index') }}" variant="ghost">Cancel</x-ui.button>
            </div>
        </form>
    </x-ui.card>
@endsection
