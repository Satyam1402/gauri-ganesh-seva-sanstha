...
@php
    ...
    $ogImage = $activity->getFirstMedia('og_image');
@endphp
    ...
    <form method="POST" action="{{ route('admin.activities.update', $activity) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <x-ui.card>
            ...
            <div class="mt-4 space-y-5">
                ...
                <x-ui.input label="Schema.org Type" name="schema_type" value="{{ old('schema_type', $seo?->schema_type ?? 'Event') }}" helper="e.g. Event, Article." :error="$errors->first('schema_type')" />
            </div>
        </x-ui.card>
