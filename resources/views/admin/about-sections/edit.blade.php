...
@section('content')
    @if ($section->key->usesOrgProfile())
        <x-ui.alert variant="info" class="mb-6">
            The structured content for this section (registration numbers, certificates, and documents) is managed on the
            <a href="{{ route('admin.org-profile.edit') }}" class="font-semibold underline">Registration &amp; Certificates</a> page.
            Use the form below only for the heading and introductory description.
        </x-ui.alert>
    @endif

    <form method="POST" action="{{ route('admin.about-sections.update', $section) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex gap-3">
            ...
            <x-ui.button href="{{ route('admin.about-sections.index') }}" variant="ghost">Back to Sections</x-ui.button>
            <x-ui.button href="{{ route('about') }}" target="_blank" variant="secondary">Preview About Page</x-ui.button>
        </div>
    </form>
@endsection
