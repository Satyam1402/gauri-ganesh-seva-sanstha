    ...
    <x-ui.card class="max-w-2xl">
        @if ($role->name === \App\Enums\Role::SuperAdmin->value)
            <x-ui.alert variant="info" class="mb-6">
                The Super Admin role always has every permission and cannot be renamed.
            </x-ui.alert>
        @endif
        ...
        <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="space-y-5">
            ...
            <x-ui.input
                ...
                required
                :readonly="$role->name === \App\Enums\Role::SuperAdmin->value"
                            ...
                            <div class="mt-1 grid grid-cols-2 gap-2">
                                @foreach ($groupPermissions as $permission)
                                    <label class="flex items-center gap-2 text-sm text-text-600 dark:text-night-text-muted">
                                        <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" @checked(in_array($permission->name, old('permissions', $rolePermissionNames))) class="rounded border-border-subtle text-primary-700 focus:ring-3 focus:ring-primary-700/35">
                                        {{ $permission->name }}
                                    </label>
                                @endforeach
                            </div>
