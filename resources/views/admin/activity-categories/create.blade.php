...
@section('title', 'Add Activity Category')
...
@section('content')
    <x-ui.card class="max-w-2xl">
        <form method="POST" action="{{ route('admin.activity-categories.store') }}" class="space-y-5">
            @csrf
            ...
            <label class="flex items-center gap-2 text-sm text-text-600 dark:text-night-text-muted">
                <input type="checkbox" name="is_active" value="1" checked class="rounded border-border-subtle text-primary-700 focus:ring-3 focus:ring-primary-700/35">
                Active
            </label>
